...
__global__ void addsa_kernel(
				...
				float *d_p,cufftComplex *d_dis0,cufftComplex *d_dis2,
				int xs,int zs,int nt,float *d_Rik
			    );
				 	...
				 	// add dispersion and attenuation terms: inside computational domain //
				  	addsa_kernel<<<dimGrid,dimBlock,0,plansm[ip].stream>>>
				  				(
				  					...
				  					singpu[ip].d_p,singpu[ip].d_dis0,singpu[ip].d_dis2,
	...
	for(ip=0;ip<GPU_N;ip++)
		 {
			shotid=singpu[ip].shotid[ns];
			strcpy(recname,"");
                	sprintf(recname,"../obsdata/reca%d_%d.bin",shotid,singpu[ip].nr);
                	fr=fopen(recname,"wb");
