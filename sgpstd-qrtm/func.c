...
extern void input_ac2d_parameters(int *shotnum,int *tmax,float *dt,float *fd,float *fo,
                                  ...
                                  float *v0,float *etas,float *etar,
                                  char fname1[40],char fname2[40],char fname3[40],
                                  char fname4[40],char fname5[40],
                                  char vpname[80],char rhoname[80],char Qpname[80],
                                  char recname[60],int *flagsource,
                                  int *nump
                                  )
{
	char strtmp[256];
	FILE *fp=fopen("../input/visac2drtm.txt","r");
	if(fp==0){printf("Cannot open ../visac2drtm.txt!\n");exit(0);}
...
extern void cal_k2d(float dx,float dz,int n0,int n1,float *k2d1,float *k2d2,float *k2d3,float *k2da)
{
    ...
    for(i=0;i<n0;i++)
    {
        ...
        {
            is=i*n1+j;
            k2da[is]=pow(kkx[i]*kkx[i]+kkz[j]*kkz[j],0.505);

	    k2d1[is]=kkx[i]*kkx[i]+kkz[j]*kkz[j];
	    k2d2[is]=pow(k2d1[is],1.0/32.0);
	    k2d3[is]=pow(k2d1[is],33.0/32.0);
        }
    }
